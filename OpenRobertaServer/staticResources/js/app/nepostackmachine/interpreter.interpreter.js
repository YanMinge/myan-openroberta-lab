        ...
        Interpreter.prototype.evalSingleOperation = function (s, n, stmt) {
            ...
            if (stmt === undefined) {
                ...
                switch (opCode) {
                    ...
                    case C.MATATABOT_MOTION_FORWARD_STEP: {
                        return [n.matatabotMotionForwardStep(stmt[C.NAME], stmt[C.STEP]), true];
                    }
                    case C.MATATABOT_MOTION_BACKWARD_STEP: {
                        return [n.matatabotMotionBackwardStep(stmt[C.NAME], stmt[C.STEP]), true];
                    }
                    case C.MATATABOT_MOTION_TURN_LEFT_ANGLE: {
                        n.matatabotMotionTurnLeftAngle(stmt[C.NAME], stmt[C.ANGLE]);
                        break;
                    }
                    case C.MATATABOT_MOTION_TURN_RIGHT_ANGLE: {
                        n.matatabotMotionTurnRightAngle(stmt[C.NAME], stmt[C.ANGLE]);
                        break;
                    }
                    case C.MATATABOT_MOTION_MOVING: {
                        var distance = s.pop();
                        return [n.matatabotMotionMoving(stmt[C.NAME], distance), true];
                    }
                    default:
